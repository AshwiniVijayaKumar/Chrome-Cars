...
function getEcommerceMultipleData(index)
{
...
if(localStorage.getItem("appLanguageCheck") == "sa")
   {
    ...
    sessionStorage.setItem("AppPageName","eecommerce");
  ...
  /*  if(localStorage.getItem("fooduserid"))
    ...
    sessionStorage.setItem("eecommerceIndex",index);
    ...
    }else
    {
    	  ...
    	  var html2='';
    	 $ecomData= $(masterData).find("ecomm[indexval=" + index + "]");
    	 
    	 //alert(JSON.stringify($ecomData));
    	if($ecomData.find("ecomsId").text().indexOf(",")==-1)
    	 {
    		 var ecomArray = $ecomData.find("ecomsId").text();
    	 ...
    	 else
    	 {
    		 ...
    		 var ecomArray = $ecomData.find("ecomsId").text().split(",");
