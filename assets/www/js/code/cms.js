...
function getCmsMultipleData(index)
{
    if(localStorage.getItem("appLanguageCheck") == "sa")
   {
    ...
    sessionStorage.setItem("AppPageName","cms");
  ...
  /*  if(localStorage.getItem("fooduserid"))
    ...
    sessionStorage.setItem("cmsIndex",index);
    ...
    }else
    {
    	  ...
    	  var html2='';
    	 $ecomData= $(masterData).find("cms[indexval=" + index + "]");
    	 
    	 //alert(JSON.stringify($ecomData));
    	 if($ecomData.find("cmsId").text().indexOf(",")==-1)
    	 {
    		 var ecomArray = $ecomData.find("cmsId").text();
    	 ...
    	 else
    	 {
    		 ...
    		 var ecomArray = $ecomData.find("cmsId").text().split(",");
