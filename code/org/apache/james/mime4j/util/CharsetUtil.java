...
public class CharsetUtil
{
  ...
  static
  {
    ...
    Charset[] arrayOfCharset = JAVA_CHARSETS;
    int j = arrayOfCharset.length;
    int i = 0;
    for (;;)
    {
      Charset localCharset;
      if (i < j) {
        localCharset = arrayOfCharset[i];
      }
      try
      {
        Object localObject1 = localCharset.canonical;
        new String(new byte[] { 100, 117, 109, 109, 121 }, (String)localObject1);
        decodingSupported.add(localCharset.canonical.toLowerCase());
        try
        {
          "dummy".getBytes(localCharset.canonical);
          encodingSupported.add(localCharset.canonical.toLowerCase());
          i += 1;
          continue;
          charsetMap = new HashMap();
          arrayOfCharset = JAVA_CHARSETS;
          int k = arrayOfCharset.length;
          i = 0;
          while (i < k)
          {
            localCharset = arrayOfCharset[i];
            ...
            {
              localObject1 = localCharset.aliases;
              int m = localObject1.length;
              j = 0;
              while (j < m)
              {
                Object localObject2 = localObject1[j];
                charsetMap.put(((String)localObject2).toLowerCase(), localCharset);
                j += 1;
              }
            }
            i += 1;
  ...
  public static Charset getCharset(String paramString)
  {
    ...
    {
      log.info("Illegal charset " + str + ", fallback to " + "ISO-8859-1" + ": " + paramString);
      return Charset.forName("ISO-8859-1");
    }
    catch (UnsupportedCharsetException paramString)
    {
      log.info("Unsupported charset " + str + ", fallback to " + "ISO-8859-1" + ": " + paramString);
    }
    return Charset.forName("ISO-8859-1");
  }
...
/* Location:              C:\Users\ADMIN\Desktop\foss\dex2jar-2.0\classes-dex2jar.jar!\org\apache\james\mime4j\util\CharsetUtil.class
